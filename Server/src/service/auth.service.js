...
import { oneMinuteExpiry, generateSixDigitOTP } from "../constant/otp.js";
...
export const loginUser = async ({ email, password }) => {
    ...
    if (!user.isAccountVerified) {
        ...
        if (!verifyOTP) {
            ...
            verifyOTP = await VerificationOTP.findOneAndUpdate({ user: user._id }, { otp: generateSixDigitOTP(), timestamp: new Date(), attempts: 0 }, { upsert: true, setDefaultsOnInsert: true, new: true });
        }
        await sendEmail(user.email, "Verify Your Email", verifyAccountTemplate(verifyOTP?.otp));
